...
![Markdowm Image][/assets/images/official2048square.jpg]

## Introduction:

The first class project for CMSC436 was to recreate the popular mobile game 2048, a game compared to the popular mobile game Flappy Bird by several commentators for its virality. The game is played on a 4x4 grid with every turn resulting in a new 2 or 4 tile to be created on random blank tile. The goal is to collaspe the tiles together in a direction, using the 4 arrow keys. Like values will be combined together to create a new tile. The goal of the game is to reach the 2048 tile before filling up the board.
