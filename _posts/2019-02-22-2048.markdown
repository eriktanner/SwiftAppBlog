...
<div class="side-by-side" style="margin-top: 0px; margin-bottom: 0px;">
    ...
    <div class="toright" style="padding-top: 12px;">
        <p> To fill out the grid, I decided to create a custom type, GridButton, which derives from UIView, a type which holds transform information and has properties such as background color. GridButton is a UIView with a Label centered, and functions which calculate it's backend value and update its display when the board is collasped.</p>
    </div>
</div>

<div style="max-height: 300px !important; overflow-y: auto !important;">
{% highlight swift %}
...
<div class="side-by-side" style="margin-top: 0px; margin-bottom: 0px;">
    ...
    <div class="toright" style="padding-top: 12px;">
        <p> To control the board, I decided to create a custom type, ArrowButton, which derives from UIButton, a type which registers touch events. ArrowButton is a UIButton with an UIImageView centered, and a function which handles the direction the board collaspes on a touch event.</p>
    </div>
</div>
...
class ArrowButton : UIButton {
    ...
    func setupArrow(arrowDelegate: ArrowDelegate, dir: String, button: ArrowButton) {
        delegate = arrowDelegate
        self.direction = dir
        
        switch (dir) {
        case "left":
            self.arrowImage.transform = self.transform.rotated(by: CGFloat(Double.pi))
            break
        case "up":
            self.arrowImage.transform = self.transform.rotated(by: CGFloat(Double.pi * 3/2))
            break
        case "down":
            self.arrowImage.transform = self.transform.rotated(by: CGFloat(Double.pi / 2))
            break
        default:
            break
        }
        
        button.addTarget(self, action: #selector(arrowClicked), for: .touchUpInside)
    }
    
    @objc func arrowClicked(_ sender: ArrowButton) {
